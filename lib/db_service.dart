...
//                          Primary key into merchant_map and the grouping
//                          key for top-merchants / recurring detection.
//   • merchantDisplay(s) → title-cased canonical form. Applied on every
//                          txn write so Activity, Top Merchants, and the
//                          Edit sheet all show consistent casing.
//
// Schema preserved (zenith_v7.db, version 1). No migration needed:
// old txn rows keep their original case in txns.merchant — new writes
// (and re-saves via the Edit sheet) gradually canonicalize them.
// ═══════════════════════════════════════
...
class Db {
  ...
  // True when no row exists (default for new merchants) or row has
  ...
  // merchants never auto-learned through the notification path.
